...
for ff=1:8  % load the test images
    ...
    for iterate=1:numel(myrate)
        ...
        lambda=0.02;
        alpha=[1/N, 1/N, 1/N];
        beta=[0,0,0];  % which decides using LRTC-TV-II ([1,1,0]) or STDC ([0,0,0])
        ...
        store_name=sprintf('pure_STDC_restore_%s.mat',filename{ff});
        ...
        save C2F_STDC_Results rse_r r_n psnr_r diff_r Z_LRTCTV2_completepatch Z_LRTCTV2_completewhole B_padimage tmpRSE_LRTC_TV2 tmpPSNR_LRTC_TV2
