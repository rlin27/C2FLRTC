...
function [tensor_Z  ] = LRTC_TV_II(index, value, lambda_1,lambda_2, alpha, beta, tsize, N ,ite_num)
    Q=cell(N,1);
    F=cell(N,1);
    R=cell(N,1);
    U=cell(N,1);
...
%         tensor_G=tensor(myG);
        ...
        if(iteration>ite_num)
            break;
        end
        
        %double(tensor_Z)
        figure(3);imshow(double(tensor_Z)); title('Image completion by pure LRTC method')
