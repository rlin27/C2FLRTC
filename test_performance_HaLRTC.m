...
for ff=1:8  % load the test images
    ...
    myrate=0.70:0.10:0.90;  % set the missing rate
    ...
    for iterate=1:numel(myrate)
        ...
        %%%%%%%%%% LRTC completion %%%%%%%%%%
        Omega = (B_pad ~= 0);
        [Z_TRLRTV2_temp,~] = HaLRTC(B_pad, Omega, alpha, rho, maxIter, epsilon);
        ...
        store_name=sprintf('pure_HaLRTC_restore_%s.mat',filename{ff});
            ...
            for row_wise=1:2^(i-1)
                for col_wise=1:2^(i-1)
                    ...
                    if i<6
                        ...
                        rho = 0.001;
                        Omega = (CCCC ~= 0);
                        [Z_TRLRTV2_patch,~] = HaLRTC(CCCC, Omega, alpha, rho, maxIter, epsilon);
                    else
                        %%%%%%%%%% LRTC completion %%%%%%%%%%
                        Omega = (B_temp ~= 0);
                        [Z_TRLRTV2_patch,~] = HaLRTC(B_temp, Omega, alpha, rho, maxIter, epsilon);
        ...
        save C2F_HaLRTC_Results rse_r r_n psnr_r diff_r Z_LRTCTV2_completepatch Z_LRTCTV2_completewhole B_padimage tmpRSE_LRTC_TV2 tmpPSNR_LRTC_TV2
