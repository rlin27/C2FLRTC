...
for ff=1:8  % load the test images
    ...
    for iterate=1:numel(myrate)
        ...
        T = zeros(II);
        T(Q) = double(B_pad(Q));
        [Z_TRLRTV2_temp histo]=LRTV_pds(prox_dom(T,dom),Q,al,tvw,be,lrw,delta,dom/255,Gam,maxiter,tol,verb);
        ...
        store_name=sprintf('pure_LRTV_PDS_restore_%s.mat',filename{ff});
        save(store_name,'Z_TRLRTV2_temp')
        
        % coarse level completion
        figure(4);
        imshow(Z_TRLRTV2_temp); title('Image Completed by pure LRTV-PDS method')
            ...
            for row_wise=1:2^(i-1)
                for col_wise=1:2^(i-1)
                    ...
                    if i<6
                        ...
                        T = zeros(II);
                        T(Q) = double(B_temp(Q));
                        Z_TRLRTV2_patch=LRTV_pds(prox_dom(T,dom),Q,al,tvw,be,lrw,delta,dom/255,Gam,maxiter,tol,verb);
                    else
                        %%%%%%%%%% LRTV_PDS completion %%%%%%%%%%
                        Q = (B_temp ~= 0);
                        II = size(B_temp);
        ...
        save C2F_LRTV_PDS_Results rse_r r_n psnr_r diff_r Z_LRTCTV2_completepatch Z_LRTCTV2_completewhole B_padimage tmpRSE_LRTC_TV2 tmpPSNR_LRTC_TV2
